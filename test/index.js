...
describe('scirichon-crud-handler', () => {
  ...
  it('add itservice', async () => {
    ...
    const response = await request.post(`/api/it_services/service`).send(getBody(it_service)).set(tokenHeaderName, internalToken)
  ...
  it('add os', async () => {
    ...
    const response = await request.post(`/api/cfgItems`).send(getBody(os)).set(tokenHeaderName, internalToken)
    assert.equal(response.statusCode, 200)
  })
  ...
  it('add physicalServer', async () => {
    ...
    const response = await request.post(`/api/cfgItems`).send(getBody(physicalServer)).set(tokenHeaderName, internalToken)
    assert.equal(response.statusCode, 200)
  })
  ...
  it('update physicalServer', async () => {
    ...
    let response = await request.put(`/api/cfgItems/${physicalServer.uuid}`).send(getBody(patch_obj)).set(tokenHeaderName, internalToken)
    assert.equal(response.statusCode, 200)
  })
